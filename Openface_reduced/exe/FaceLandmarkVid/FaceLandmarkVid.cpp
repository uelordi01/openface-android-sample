                        ...
                        double cx, double cy)
{
	...
	if (detection_certainty < visualisation_boundary)
	{
        ...
        cv::Vec6d pose_estimate_to_draw = LandmarkDetector::GetPose(face_model,
                                                                    fx, fy,
                                                                    cx, cy);

        // Write the pose values in the image
        std::stringstream ss;
        ss << "TX: " << pose_estimate_to_draw[0];
        putText(captured_image, ss.str(), cv::Point(10, 60),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
        ss << "TY: " << pose_estimate_to_draw[1];
        putText(captured_image, ss.str(), cv::Point(10, 80),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
        ss << "TZ: " << pose_estimate_to_draw[2];
        putText(captured_image, ss.str(), cv::Point(10, 100),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
        ss << "RX: " << pose_estimate_to_draw[3] * RAD2DEG;
        putText(captured_image, ss.str(), cv::Point(10, 120),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
        ss << "RY: " << pose_estimate_to_draw[4] * RAD2DEG;
        putText(captured_image, ss.str(), cv::Point(10, 140),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
        ss << "RZ: " << pose_estimate_to_draw[5] * RAD2DEG;
        putText(captured_image, ss.str(), cv::Point(10, 160),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(0, 255, 255));
        ss.str("");
...
//                                  thickness, fx, fy, cx, cy);
	...
	string fpsSt("FPS:");
	fpsSt += fpsC;
    cv::putText(captured_image, fpsSt, cv::Point(10, 20),
                CV_FONT_HERSHEY_SIMPLEX, 0.5, CV_RGB(255, 0, 0));
...
int main (int argc, char **argv)
{
    ...
    // Include the focal length values using the projection angle
//    int imageWidth = 640;
//    int imageHeight = 480;
//    const float aspect = imageWidth / imageHeight;
//    const float fovy = 45;
//    float fovGraVer = fovy;
//    float fovGraHor = fovGraVer * aspect;
//    float fovRadHor = fovGraHor * static_cast<float>(DEG2RAD);  // 0.7853975
//    float fovRadVer = fovGraVer * static_cast<float>(DEG2RAD);
//    fx = imageWidth / (2.0 * tan(fovRadHor / 2.0));
//    fy = imageHeight / (2.0 * tan(fovRadVer / 2.0));
    ...
    std::cout << "Loaded camera configuration:" << std::endl;
	...
	while(!done) // this is not a for loop as we might also be reading from a webcam
	{
		...
		if(files.size() > 0)
		{
            ...
            std::cout << "Input video file: '" << current_file << "'"
                      << std::endl;
		}
		else
		{
        ...
        bool onlineCameraCapture = false;
		if( current_file.size() > 0 )
		{
            ...
            current_file = getFilePath(current_file);
		...
		{		
            ...
            // Flip the image if a camera is used for the capture
            if (onlineCameraCapture) {
