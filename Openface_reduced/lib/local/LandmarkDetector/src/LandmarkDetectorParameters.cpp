...
// true if file exists
bool fileExists2(const std::string& file) {
    struct stat buf;
    return (stat(file.c_str(), &buf) != -1);
}

std::string getFilePath2(const std::string& filename) {
...
FaceModelParameters::FaceModelParameters(vector<string> &arguments)
{
    ...
    std::string root = getFilePath2(arguments[0]);
    ...
    std::string config_path = getFilePath2(CONFIG_DIR);
    std::string model_path = getFilePath2(model_location);
    if (fileExists2(model_path))
	{
        model_location = model_location;
	}
    else if (fileExists2(root + "/" + model_path))
    {
        model_location = (root + "/" + model_location);
    }
    else if (fileExists2(config_path + "/" + model_path))
	{
        model_location = (config_path + "/" + model_location);
	}
	else
	{
